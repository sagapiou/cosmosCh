...
func (k *Keeper) MustRefundWager(ctx sdk.Context, storedGame *types.StoredGame) {
	if storedGame.MoveCount == 1 {
		...
		// Refund
		red, err2 := storedGame.GetRedAddress()
		if err2 != nil {
			panic(err2.Error())
		}
		err2 = k.bank.SendCoinsFromModuleToAccount(ctx, types.ModuleName, red, sdk.NewCoins(storedGame.GetWagerCoin()))
		if err2 != nil {
			panic(fmt.Sprintf(types.ErrCannotRefundWager.Error(), err2.Error()))
		}
	}
}